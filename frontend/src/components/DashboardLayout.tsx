...
import { CiHome,CiHeart,CiBookmark, CiSearch} from "react-icons/ci";
...
export const DashboardLayout = ({
	...
	const navigation = [
		{ name: "My Images", href: "/dashboard", icon: CiHome, current: true },
		{
			name: "My favorite images",
			href: "/dashboard/favorites",
			icon:  CiBookmark, //BookmarkIcon,
			current: false,
		},
		{
			name: "Iamges I like",
			href: "/dashboard/likes",
			icon: CiHeart, // corazon
			current: false,
		},
		{
			name: "Comments",
			href: "/dashboard/comments",
			icon: LiaComment, // corazon
			current: false,
		},
	];
							...
							<div className='relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-[#16a084]/20 backdrop-blur-md     '>
								<Transition.Child
									...
									<div className='absolute top-0 right-0 -mr-12 pt-2'>
										<button
											type='button'
											className='ml-1 flex items-center justify-center h-10 w-10 rounded-full focus:outline-none focus:ring-2 focus:ring-inset focus:ring-black cursor-pointer '
...
{/* 									<Image
				...
				<div className='md:pl-64 flex flex-col flex-1'>
					<div className='sticky top-0 z-10 flex-shrink-0 flex h-12  shadow'>
						<button
							type='button'
							className='px-4 text-black   md:hidden'
							onClick={() => setSidebarOpen(true)}>
							<span className='sr-only'>Open sidebar</span>
							{/* <MenuIcon className='h-6 w-6' aria-hidden='true' /> */}
						</button>
						<div className='flex-1 px-4 flex justify-between bg-gray-100 text-black backdrop-blur-2xl border-b '>
...
{/* 											<MagnifyingGlassIcon
										...
										<input
											id='search-field'
											className='block w-full h-full pl-8 pr-3 py-2 border-transparent  focus:outline-none focus:placeholder-black text-black focus:ring-0 focus:border-transparent sm:text-sm'
											placeholder='Search'
											type='search'
											name='search'
...
{/* 											<Image
					...
					<main className='flex-1 py-6 w-full  px-12 '>
							{children}
					</main>
				</div>
			</div>
